...
def make_plot() -> Path:
    ...
    # One compact annotation box with two arrows, one per marker
    x_right = ax_pm.get_xlim()[1]
    x_box = NABEL_PM10 + (x_right - NABEL_PM10) * 0.30
    y_box = z_min + z_range * 0.22
 
    ax_pm.annotate(
        "",
        xy=(NABEL_PM25, nabel_alt_pm25),
        xytext=(x_box, y_box),
    ...
    ax_pm.text(
        x_box, y_box,
        f"NABEL Zürich-Kaserne\n14:30 local time\n"
        f"PM2.5 = {NABEL_PM25} µg m⁻³\n"
        f"PM10  = {NABEL_PM10} µg m⁻³",
        fontsize=7.5,
        color="#374151",
        ha="center",
        va="bottom",
        bbox=dict(facecolor="white", edgecolor="#9ca3af", alpha=0.93,
    ...
    ax_pm.legend(loc="upper right", frameon=True, framealpha=0.9, fontsize=8)
